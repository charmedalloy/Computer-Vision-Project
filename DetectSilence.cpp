...
#include <iostream>
#include <stdio.h>
#include "/OpenCV/opencv/build/include/dirent.h"
...
bool detectSilence(Mat frame, Point location, Mat ROI, CascadeClassifier cascade, bool video)
{
	...
	if (!video) {
		...
		cascade.detectMultiScale(ROI, mouths, 1.1, 30, 0, Size(20, 20));
	}
...
int detect(Mat frame, CascadeClassifier cascade_face, CascadeClassifier cascade_mouth, bool video) {
	Mat frame_gray;
	vector<Rect> faces;
	...
	*/
	...
	for (int i = 0; i < nfaces; i++) {
		...
		if (detectSilence(frame, Point(x1, y1), lower_faceROI, cascade_mouth, video)) {
